...
import torch 
import numpy as np
from dataset import Seq_Median_Nerve_Dataset 
...
def loop(model, loader, optimizer, criterion, args, mode='train'): 
    ...
    for idx, (x, y) in pbar: 
        index = torch.randint(0, 290, (args.num_idxs,), device=args.device)
        x = x.to(args.device)
        y = y.to(args.device)
        y = y[:, index]
        x = x.repeat(1, args.num_idxs, 1, 1)
        y = rearrange(y, 'b c h w -> (b c) 1 h w')
        x = rearrange(x, 'b c h w -> (b c) 1 h w')
        index = rearrange(index, 'b -> b 1 1 1')
...
def train(model, loaders, optimizer, criterion, args):
    ...
    for epoch in range(args.epochs): 
        ...
        print(log_dct)
        save_model(model, log_dct, args)
        if loss < best_loss: 
            best_loss = loss 
            args.early_stopping_idx = 0 
            save_model(model, log_dct, args, best=True)
...
def main(args):
    dataset = Seq_Median_Nerve_Dataset(args.data_path)
    ...
    if args.model_name == 'q_unet':
        from models import Seq_Q_UNET
        model = Seq_Q_UNET(in_ch=args.in_ch, out_ch=args.out_ch, \
                 n_qubits=args.n_qubits).to(args.device)
    ...
    if not os.path.exists('./ckpts/sequential'):
        os.makedirs('./ckpts/sequential')
    if not os.path.exists('./res/sequential'):
        os.makedirs('./res/sequential')
    args.save_path = f'./ckpts/sequential/{args.model_name}'
    args.save_best_path = f'./ckpts/sequential/best_{args.model_name}'
    
    optimizer = Adam(model.parameters(), lr=args.lr)
    criterion = SSIM_DICE_BCE() 
    
    f = open(f'./res/sequential/{args.model_name}_train_plot.csv', 'w')
    ...
    print(vars(args))
    with open('./res/sequential/config.txt', 'w') as config:
        config.write(str(vars(args)))

    train(model, loaders, optimizer, criterion, args)
    f.close() 

    plot('./res/sequential/{}_train_plot.csv'.format(args.model_name),
        './res/sequential/{}_train_plot.png'.format(args.model_name))
...
if __name__ == '__main__': 
    ...
    parser.add_argument('-m', '--model_name', type=str, default='q_unet')
    parser.add_argument('-bs', '--batch_size', type=int, default=1)
    parser.add_argument('-ix', '--num_idxs', type=int, default=16)
